...
\author[
addressref={aff6},
   email={lf.rosalesm@up.edu.pe}   % email address
]{Francisco Rosales} 
\author[
addressref={aff1},
]{Ngoc-Thuy Ha}
...
\address[id=aff5]{%                           % unique id
	\orgname{IT Solutions for Animal Production (vit), Verden}, % university, etc                    %
	%\postcode{}                                % post or zip code                              % city
	\cny{Germany}                                    % country
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The extended RRBLUP which includes interactions is not invariant to translations, since it does not only penalize interactions (degree 2), but also additive effects (degree 1). Our observation are  valid for general penalized regression settings, for instance, the $\ell_1$ penalized LASSO method.
